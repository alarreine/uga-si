...
void main() {
  ...
  float depthTex = texture(shadowmap,shadcoord.xy).x;

  //float test = texture(shadowmap,vec3(shadcoord));
  //if(shadcoord.z > depthTex)
  if(shadcoord.z > depthTex+0.005) //shadow acne
      color /= 2;
  outBuffer  = color*v;
}
