...
void Viewer::createTextures() {
   QImage image = QGLWidget::convertToGLFormat(QImage("textures/tex1.jpg"));
  ...
  //Which color do we apply if several texels are projected on one fragments
  glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_LINEAR);// choose the filtering mode for minification
  ...
  //Que pasa con la imagen cuando se expande, se estira, se repite , es un espejo
...
void Viewer::enableShader(unsigned int shader) {
  ...
  glBindTexture(GL_TEXTURE_2D,_texIds[0]); // activate the texture using the appropriate ID (in the texture unit 0)
  glUniform1i(glGetUniformLocation(id,"tex"),0);// el parametro 0 va a cambiar cuando si vamos agregando texturas
